...
Keyword search across **10,000 entries in <100 ms** (measured first — the
baseline below is the reference point).

## Baseline — keyword search (recorded 2026-08-14)
...
**Conclusion:** the baseline (~3 ms) already sits ~30× under the <100 ms
target; re-measure once semantic re-ranking lands.

## Capture latency — baseline (recorded 2026-08-14)
...
Target: <100 ms per capture with git context.
...
**Optimization leads:** replace the four git invocations with a single
`git status --porcelain=v2 --branch` call, cache git detection per
directory, and skip the `--show-toplevel` call when the cwd is unchanged.

## Semantic layer — baselines (recorded 2026-08-14)
...
Target ("vector queries must stay interactive on 10k entries"): met with
large margin — semantic and hybrid stay far below 100 ms.
...
## Shell & git integration — baselines (recorded 2026-08-15)

Machine: Windows 11, 16 logical CPUs, release build
(`scripts/bench_shell_git.ps1` + `examples/bench_sanitize.rs`).
...
Budget: the prompt must add <50 ms to the shell path. Measured overhead
is ~100x under budget — the hook is invisible to typing latency.

**`recall capture --from-shell` end-to-end** (spawned release binary,
snapshot env vars set, git context present, 20 runs):

- **118.3 ms/capture** — statistically the same as the plain capture
  baseline (117.3 ms); the snapshot read + sanitization scan cost is
  within noise. Process startup and the four git subprocess calls
  continue to dominate (unchanged from the capture-baseline findings).
...
**Conclusion:** every latency budget is met with orders of magnitude to
spare; no optimization was performed because none is warranted by the
measurements.

## Project-scoped search — baselines (recorded 2026-08-15)
...
- Semantic numbers here are higher than the semantic-layer baseline
  (~9.8 ms): the second lookup joins `memories` for the status/project
...
- Beyond 10k the trend is documented, not hidden: hybrid crosses
  ...
  everything stays interactive and the rule is to fix only measured
  breakage (ADR-0025). Ranked options for >30k stores are documented
  (project scoping, vec0 auxiliary indexes).
...
- Baselines reproduced ahead of the full-scale run:
  `target/scale-baseline-*.txt` (FTS 3.3–4.0 ms, semantic 17.4–19.0 ms,
