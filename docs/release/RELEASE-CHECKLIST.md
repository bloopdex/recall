...
- release artifacts (binaries, SHA256SUMS, install-script copies) are
  generated locally during a release by `scripts/release.ps1` into
  `dist/`, which is gitignored;
- a release is published by creating a GitHub Release for the `v<version>`
  tag and attaching the generated bundle files;
...
- [ ] `git tag v<version>` on the verified commit; `git status` clean
      (the generated `dist/` bundle must NOT appear in `git status` —
      it is gitignored).
- [ ] Create the GitHub Release for the tag and attach the generated
      bundle files (binary, SHA256SUMS, install scripts, CHANGELOG,
      LICENSE). Publication is currently pending a hosting decision;
      until it is resolved, the generated `dist/` bundle is the release
      artifact.
